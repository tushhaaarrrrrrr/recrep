...
class AdminCog(commands.Cog):
    ...
    async def set_approval(self, interaction: discord.Interaction, channel: discord.TextChannel):
        ...
        logger.info(f"Guild {interaction.guild_id}: Approval channel → {channel.id}")
    ...
    async def set_log_channel(
        ...
        logger.info(f"Guild {interaction.guild_id}: {log_type.name} log channel → {channel.id}")
        await interaction.response.send_message(
    ...
    @app_commands.command(
        name="role",
        description="Grant, revoke, or list internal staff roles (admin, comayor, builder, recruiter)"
    )
    @app_commands.default_permissions(administrator=True)
    ...
    @app_commands.command(
        name="view_config",
        description="Show the current channel configuration and internal roles"
    )
    @app_commands.default_permissions(administrator=True)
