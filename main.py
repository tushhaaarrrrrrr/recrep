...
from cogs.supplier import SupplierCog
...
class TownyBot(commands.Bot):
    ...
    async def keep_db_alive(self):
        ...
        try:
            ...
            self.logger.debug("DB keepalive failed", exc_info=True)
    ...
    async def setup_hook(self):
        ...
        try:
            ...
            self.logger.critical("Failed to initialize services")
            await self.close()
            sys.exit(1)

        # Load all cogs with per-cog error isolation
        cogs = [
            RecruitmentCog,
            ProgressCog,
            InvoiceCog,
        ...
        for cog_cls in cogs:
            try:
                ...
                self.logger.info("Loaded cog: %s", cog_cls.__name__)
            except Exception:
                self.logger.exception("Failed to load cog: %s", cog_cls.__name__)
                raise
